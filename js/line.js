...
// 서로 독립적으로 작용하므로 위의 변수를 활용 못함.


// 언어상자 - Mobile의 footer 영역
$(function(){
	...
	$(window).one("load",function(){ // 딱 한번만 실행되는 .one() 메소드
		...
		// .before() 메서드 : 선택한 요소 이전 위치에 새 요소 생성 
		// .after() 메서드 : 선택한 요소 다음 위치에 새 요소 생성 
		$(".langlist>li>a").each(function(idx){ // .each - 여러개의 요소들에 순차적으로 하나씩 접근 
			var optCont='<option value="'+idx+'">'+$(this).text()+'</option>';			
			$("#langsrv").append(optCont); // .append(); - 선택한 요소의 마지막 위치에 새 클래스 생성 
		});
	});
});
